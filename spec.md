...
- [Introduction](#introduction)
  - **[What is UnMarkDocs Flavored Markdown?](#what-is-unmarkdocs-flavored-markdown)**
  - [What is Markdown?](#what-is-markdown)
  - [Why is a spec needed?](#why-is-a-spec-needed)
  - [About this document](#about-this-document)
  - **[About the examples on this document](#about-the-examples-on-this-document)**
...
- [Leaf blocks](#leaf-blocks)
  ...
  - **[Alerts (extension)](#alerts-extension)**
...
## What is UnMarkDocs Flavored Markdown?

UnMarkDocs Flavored Markdown, sometimes shortened as UFM, is the dialect of Markdown that is currently supported for content rendered on [UnMarkDocs](https://unmarkdocs.co)(<https://unmarkdocs.co>).

This formal specification, based on the CommonMark Spec, defines the syntax and semantics of this dialect.

UFM is a strict superset of CommonMark. All the features which are supported in UnMarkDocs and that are not specified on the original CommonMark Spec are hence known as extensions, and highlighted as such.

While UFM supports a wide range of inputs, it’s worth noting that UnMarkDocs may perform additional post-processing and sanitization after UFM is converted to HTML to ensure security and consistency of the website.
...
## About the examples on this document

The HTML examples on this document may not exactly match the ones you'll get when using UnMarkDocs. This is because we apply some extra CSS classes, HTML properties... to prettify the output.
...
UFM enables the table extension, where an additional leaf block type is available.
...
UFM adds a new type of block, that can be used to show warnings, success messages, tips...

The alert syntax is as follows:

- Three [exclamation marks] (`!`).
- A [space] and the alert type (one of `success`, `note`, `info`, `panel`, `danger`).
- A line break.
- An (optional) [number sign] (`#`), followed by an space and the alert title, followed by a line break.
- The alert body, that can contain any ASCII text, and cannot start with a [number sign] (`#`).
- A line break.
- Three [exclamation marks] (`!`).
...
```````````````````````````````` example
!!! success
# Something went really well
Something you did went really well.
Congratulations!
!!!

<div><h4>Something went really well</h4><p>Something you did went really well.<br>Congratulations!</p></div>
````````````````````````````````

Here is another example without a title:

```````````````````````````````` example
!!! warning
You should retry that again...
!!!

<div><p>You should retry that again...</div>
````````````````````````````````
...
UFM adds the abbreviation extension, which allows you to add additional information to your abbreviations. The syntax is as follows:

[asterisk] (`*`) followed by the abbreviation inside [square brackets] (`[]`), a [colon] (`:`) and the abbreviation meaning inside [square brackets] (`[]`).
...
<p><img src="https://assets-cdn.github.com/images/icons/emoji/unicode/1f44d.png?v7"></img></p>
````````````````````````````````
...
UFM enables the strikethrough extension, where an additional emphasis type is available.
...
<p>This <del>text</del><del> is </del><del>curious</del>.</p>
...
## Link Generation (extension)

UFM allows you to specify an [at] character (`@`) to generate a link to the current page linking to the slug of the title of the link.

```````````````````````````````` example
[Example link](@)
.
<p><a href="#example-link">Example Link</a></p>
````````````````````````````````
...
raw HTML tag and will be escaped with the exception of [HTML comments](@).
